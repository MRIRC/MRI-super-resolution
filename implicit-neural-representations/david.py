		...
		for direction in range(3):  # gradient directions x, y, z
			...
			for acq in range(starts[direction], ends[direction]):
				...
				for inx, metric in enumerate(metrics):
					f.write('{},{},{},{},{}\n'.format(pt_no,'DWI', directions[direction], acq, metric, 
																	calculate_contrast(case, 1, img, 0)[inx]))
					f.write('{},{},{},{},{}\n'.format(pt_no,'ADC', directions[direction], acq, metric, 
																	calculate_contrast(case, 1, adc_img, 0)[inx]))
				
				accept = case.accept[:, :, _slice, acq]
				img_dataset.append(Image.fromarray(img))
				accept_weights.append(accept)
				sum_image += img
				sum_accepted += image*accept
				sum_accepts += accept
				ctr += 1
			...
			accepted_mean_adc = -np.log((accepted_mean/(b0_scaled + eps)) + eps)/b 
			accepted_mean_adc *= 1000
			direction_mean_adc = -np.log((direction_mean/(b0 + eps)) + eps)/b
			direction_mean_adc *= 1000
			
			for inx, metric in enumerate(metrics):
				f.write('{},{},{},{},{}\n'.format(pt_no,'DWI', directions[direction], 'mean', metric, 
																	calculate_contrast(case, 1, direction_mean, 0)[inx]))
				f.write('{},{},{},{},{}\n'.format(pt_no,'ADC', directions[direction], 'mean', metric, 
																	calculate_contrast(case, 1, direction_mean_adc, 0)[inx]))

				f.write('{},{},{},{},{}\n'.format(pt_no,'DWI_ERD', directions[direction], 'mean', metric, 
																	calculate_contrast(case, 1, accepted_mean, 0)[inx]))
				f.write('{},{},{},{},{}\n'.format(pt_no,'ADC_ERD', directions[direction], 'mean', metric, 
																	calculate_contrast(case, 1, accepted_mean_adc, 0)[inx]))		
