										...
										os.mkdir(exp)
											...
											for step in tqdm.tqdm(range(args.steps)):
												for sample in range(len(dataset)):
													...
													model_output, coords, img_grad = net(model_input)
													...
													if not step % 25 and sample==11:
														...
														grad_filename = os.path.join(exp, 'grad_' + str(step) + '.eps')
														save_fig(predicted,img_filename,size=(2,2),dpi=600)
														save_fig(orig,grad_filename,cmap='hot')
